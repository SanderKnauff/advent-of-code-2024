...
//            });
    ...
    Vec2d simulateMovement(Grid2d<WarehouseItem> warehouse, Vec2d initialRobotLocation, Direction movement) {
        final var target = movement.moveTo(initialRobotLocation, warehouse).orThrow();

        final var targetType = warehouse.get(target);
        final var pushResult = switch (targetType) {
            case WALL -> new PushResult.Failure();
            case EMPTY -> new PushResult.Success(Set.of());
            case BOX -> {
                final var boxPushResult = tryPush(warehouse, target, movement);
                yield switch (boxPushResult) {
                    case PushResult.Failure () -> new PushResult.Failure();
                    case PushResult.Success s -> new PushResult.Success(s.locationsToPush());
                };
            }
            case ROBOT -> throw new IllegalStateException("Robot tried moving into itself. Robot: [%s], target: [%s]".formatted(initialRobotLocation, targetType));
        };

        if (!(pushResult instanceof PushResult.Success s)) {
            return initialRobotLocation;
        }

        for (Vec2d location : s.locationsToPush()) {
        ...
        warehouse.set(target, WarehouseItem.ROBOT);
        return target;
    }

    PushResult tryPush(Grid2d<WarehouseItem> warehouse, Vec2d box, Direction movement) {
        final var boxLeft = findLeftPartOfBox(warehouse, box);
    ...
    private PushResult pushVertical(Grid2d<WarehouseItem> warehouse, Direction movement, Vec2d boxLeft) {
        ...
        final var allPushed = result.stream()
                .filter(pushResult -> pushResult instanceof PushResult.Success)
    ...
    private PushResult pushLeft(Grid2d<WarehouseItem> warehouse, Vec2d boxLeft) {
        ...
        if (!(leftPushResult instanceof PushResult.Success s)) {
            return new PushResult.Failure();
        }

        final var allPushed = new HashSet<>(s.locationsToPush());
        return new PushResult.Success(allPushed);
    }
